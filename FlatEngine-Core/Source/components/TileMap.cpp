...
namespace FlatEngine
{
	...
	void TileMap::PutData(json componentJson, std::string objectName)
	{
		...
		SetTileHeight(JsonHelper::CheckJsonInt(componentJson, "tileHeight", objectName));
		...
		if (JsonHelper::JsonContains(componentJson, "tileSets", objectName))
		{
		    for (int tileSet = 0; tileSet < componentJson.at("tileSets").size(); tileSet++)
		    {
		        ...
		        std::string tileSetName = JsonHelper::CheckJsonString(tileSetJson, "name", objectName);
		        if (TileSetManager::GetTileSet(tileSetName) != nullptr)
		        {
		            AddTileSet(tileSetName);
		        }
		        else
		        {
		            Logger::log.Err("TileSet: {} could not be found.", tileSetName);
		        }
		    }
		}
		// Get Tile data
		if (JsonHelper::JsonContains(componentJson, "tiles", objectName))
		{
		    for (int tile = 0; tile < componentJson.at("tiles").size(); tile++)
		    {
		        ...
		        if (tileSetName != "" && TileSetManager::GetTileSet(tileSetName) != nullptr && tileSetIndex != -1)
		        {
		            SetTile(Vector2(x, y), TileSetManager::GetTileSet(tileSetName), tileSetIndex);
		        }
		    }
		}
		// Get Collision Area data
		if (JsonHelper::JsonContains(componentJson, "collisionAreas", objectName))
		{
		    for (int collisionArea = 0; collisionArea < componentJson.at("collisionAreas").size(); collisionArea++)
		    {
		        ...
		        std::string collisionAreaName = JsonHelper::CheckJsonString(colliderAreaJson, "name", objectName);
		        std::vector<std::pair<Vector2, Vector2>> collisionCoordBuffer;

		        for (int colArea = 0; colArea < colliderDataJson.size(); colArea++)
		        {
		            Vector2 startCoord = Vector2(JsonHelper::CheckJsonFloat(colliderDataJson.at(colArea), "startCoordX", objectName), JsonHelper::CheckJsonFloat(colliderDataJson[colArea], "startCoordY", objectName));
		            Vector2 endCoord = Vector2(JsonHelper::CheckJsonFloat(colliderDataJson.at(colArea), "endCoordX", objectName), JsonHelper::CheckJsonFloat(colliderDataJson[colArea], "endCoordY", objectName));
		        ...
		        {
		            SetCollisionAreaValues(collisionAreaName, collisionCoordBuffer);
		        }
		    }
		}
    }
		...
		if (m_collisionAreas.count(label) >= 0)
		{
			...
			for (CollisionAreaData collisionArea : m_collisionAreas.at(label))
			{
				...
				//if (collider != nullptr)
				{
					...
					float tileMapCenterX = (float)(m_width / 2.0f);
					if (m_width % 2)
					{
						tileMapCenterX += 1;
					}
					...
					float tileMapCenterY = (float)(m_height / 2.0f);
					if (m_height % 2)
					{
						tileMapCenterY += 1;
					}
	...
	void TileMap::SetTile(Vector2 coord, TileSetManager::TileSet* tileSet, int tileSetIndex)
	{
		if (tileSet != nullptr)
		{
