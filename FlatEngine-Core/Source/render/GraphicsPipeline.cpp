...
namespace FlatEngine
{
    ...
    void GraphicsPipeline::Cleanup()
    {
        ...
        vkDestroyPipeline(DeviceManager::logicalDevice.GetDevice(), m_graphicsPipeline, nullptr);
        vkDestroyPipelineLayout(DeviceManager::logicalDevice.GetDevice(), m_pipelineLayout, nullptr);
    }
    ...
    VkShaderModule GraphicsPipeline::CreateShaderModule(const std::vector<char>& code)
    {
        ...
        if (vkCreateShaderModule(DeviceManager::logicalDevice.GetDevice(), &createInfo, nullptr, &shaderModule) != VK_SUCCESS)
        {
    ...
    void GraphicsPipeline::CreateGraphicsPipeline(RenderPass& renderPass, VkDescriptorSetLayout& descriptorSetLayout)
    {
        ...
        if (FileHelper::DoesFileExist(m_vertexPath) && FileHelper::DoesFileExist(m_fragmentPath))
        {
        ...
        {
            Logger::log.Err("Vertex shader and Fragment shader paths not found. GraphicsPipeline::CreateGraphicsPipeline() failed.");
            return;
        }
        ...
        m_viewport.width = (float)RenderWindow::window.GetExtent().width;
        m_viewport.height = (float)RenderWindow::window.GetExtent().height;
        ...
        std::vector<VkDescriptorSetLayout> layouts(VulkanManager::MAX_FRAMES_IN_FLIGHT, descriptorSetLayout);
        ...
        if (vkCreatePipelineLayout(DeviceManager::logicalDevice.GetDevice(), &m_pipelineLayoutInfo, nullptr, &m_pipelineLayout) != VK_SUCCESS)
        {
        ...
        if (vkCreateGraphicsPipelines(DeviceManager::logicalDevice.GetDevice(), VK_NULL_HANDLE, 1, &m_pipelineInfo, nullptr, &m_graphicsPipeline) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create graphics pipeline.");
        }

        // Cleanup when pipeline is finished being created
        vkDestroyShaderModule(DeviceManager::logicalDevice.GetDevice(), vertShaderModule, nullptr);
        vkDestroyShaderModule(DeviceManager::logicalDevice.GetDevice(), fragShaderModule, nullptr);
    }
