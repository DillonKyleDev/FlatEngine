...
namespace FlatEngine
{
    ...
    bool Helper::CheckDeviceExtensionSupport(VkPhysicalDevice physicalDevice)
    {
        ...
        std::set<std::string> requiredExtensions(VulkanManager::DEVICE_EXTENSIONS.begin(), VulkanManager::DEVICE_EXTENSIONS.end()); // Get list of extensions required for presenting images to window
    ...
    VkCommandBuffer Helper::BeginSingleTimeCommands(VkCommandPool& commandPool)
    {
        ...
        vkAllocateCommandBuffers(DeviceManager::logicalDevice.GetDevice(), &allocInfo, &commandBuffer);
    ...
    void Helper::EndSingleTimeCommands(VkCommandBuffer commandBuffer, VkCommandPool& commandPool)
    {
        ...
        VkResult err;
        err = vkQueueSubmit(DeviceManager::logicalDevice.GetGraphicsQueue(), 1, &submitInfo, VK_NULL_HANDLE);
        //assert(err == 0);
        vkQueueWaitIdle(DeviceManager::logicalDevice.GetGraphicsQueue());
        // Don't forget to clean up the command buffer used for the transfer operation.
        vkFreeCommandBuffers(DeviceManager::logicalDevice.GetDevice(), commandPool, 1, &commandBuffer);
    }
