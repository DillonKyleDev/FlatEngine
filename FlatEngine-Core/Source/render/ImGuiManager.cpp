...
namespace FlatEngine
{
    ...
    void ImGuiManager::Setup(VkCommandPool& commandPool)
    {
    ...
    {        
        vkDestroyCommandPool(DeviceManager::logicalDevice.GetDevice(), *m_commandPool, nullptr);

        VkResult err = vkDeviceWaitIdle(DeviceManager::logicalDevice.GetDevice());
        VulkanManager::vulkan.check_vk_result(err);
        ImGui_ImplVulkan_Shutdown();
        ImGui_ImplSDL2_Shutdown();
