...
namespace FlatEngine
{
    namespace RenderWindow 
    {
        ...
        void Window::CleanupDrawingResources()
        {
            DestroyImageViews(); // Not required but explicit
            DestroySwapChain();
        }

        void Window::SetHandles(VkCommandPool* commandPool)
        {
            m_commandPool = commandPool;
        }

        bool Window::CreateSDLWindow(std::string windowTitle, int windowWidth, int windowHeight)
        {
            bool b_success = true;
        ...
        {
            SDL_Vulkan_CreateSurface(GetWindow(), VulkanManager::vulkan.GetInstance(), &m_surface);
        }
        ...
        void Window::DestroySurface()
        {
            ...
            // {
                vkDestroySurfaceKHR(VulkanManager::vulkan.GetInstance(), m_surface, nullptr);
            // }
        }

        void Window::SetFullscreen(bool b_isFullscreen)
        {
        ...
        void Window::CreateDrawingResources()
        {
            CreateSwapChain();
            CreateImageViews();
        }
        ...
        {
            SwapChainSupportDetails swapChainSupport = Helper::QuerySwapChainSupport(DeviceManager::physicalDevice.GetDevice(), m_surface);
            ...
            VulkanManager::imageCount = swapChainSupport.capabilities.minImageCount + 1;

            // Clamp to max if there is one
            if (swapChainSupport.capabilities.maxImageCount > 0 && VulkanManager::imageCount > swapChainSupport.capabilities.maxImageCount)
            {
                VulkanManager::imageCount = swapChainSupport.capabilities.maxImageCount;
            }

            VulkanManager::MAX_FRAMES_IN_FLIGHT = VulkanManager::imageCount;

            VkSwapchainCreateInfoKHR createInfo{};
            createInfo.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR;
            createInfo.surface = m_surface;
            createInfo.minImageCount = VulkanManager::imageCount;
            ...
            QueueFamilyIndices indices = Helper::FindQueueFamilies(DeviceManager::physicalDevice.GetDevice(), m_surface);
            ...
            if (vkCreateSwapchainKHR(DeviceManager::logicalDevice.GetDevice(), &createInfo, nullptr, &m_swapChain) != VK_SUCCESS)
            {
            ...
            else
            {
                ...
                vkGetSwapchainImagesKHR(DeviceManager::logicalDevice.GetDevice(), m_swapChain, &VulkanManager::imageCount, nullptr);
                m_swapChainImages.resize(VulkanManager::imageCount);
                vkGetSwapchainImagesKHR(DeviceManager::logicalDevice.GetDevice(), m_swapChain, &VulkanManager::imageCount, m_swapChainImages.data());
            }
        }

        void Window::DestroySwapChain()
        {
            if (DeviceManager::logicalDevice.GetDevice() != VK_NULL_HANDLE)
            {
                vkDestroySwapchainKHR(DeviceManager::logicalDevice.GetDevice(), m_swapChain, nullptr);
            }
        }
        ...
        void Window::DestroyImageViews()
        {
            ...
            {
                vkDestroyImageView(DeviceManager::logicalDevice.GetDevice(), imageView, nullptr);
            }
        }
        ...
        VkSurfaceFormatKHR Window::GetSurfaceFormat()
        {
            return m_surfaceFormat;
        }

        VkPresentModeKHR Window::GetPresentMode()
        {
        ...
        VkFormat Window::GetImageFormat()
        {
        ...
        void Window::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
        {
            ...
            if (vkCreateImage(DeviceManager::logicalDevice.GetDevice(), &imageInfo, nullptr, &image) != VK_SUCCESS) {
                throw std::runtime_error("failed to create image!");
            }

            VkMemoryRequirements memRequirements{};
            vkGetImageMemoryRequirements(DeviceManager::logicalDevice.GetDevice(), image, &memRequirements);

            VkMemoryAllocateInfo allocInfo{};
            allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
            allocInfo.allocationSize = memRequirements.size;
            allocInfo.memoryTypeIndex = DeviceManager::physicalDevice.FindMemoryType(memRequirements.memoryTypeBits, properties);

            if (vkAllocateMemory(DeviceManager::logicalDevice.GetDevice(), &allocInfo, nullptr, &imageMemory) != VK_SUCCESS)
            {
                throw std::runtime_error("failed to allocate image memory!");
            }

            vkBindImageMemory(DeviceManager::logicalDevice.GetDevice(), image, imageMemory, 0);
        }

        void Window::CreateImageView(VkImageView& imageView, VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
        {
            ...
            if (vkCreateImageView(DeviceManager::logicalDevice.GetDevice(), &viewInfo, nullptr, &imageView) != VK_SUCCESS)
            {
                throw std::runtime_error("failed to create texture image view!");
            }
        }

        void Window::CreateTextureSampler(VkSampler& textureSampler, uint32_t mipLevels)
        {
            ...
            samplerInfo.minFilter = VK_FILTER_LINEAR; // VK_FILTER_NEAREST for pixel art; // VK_FILTER_LINEAR for non pixel art
            ...
            vkGetPhysicalDeviceProperties(DeviceManager::physicalDevice.GetDevice(), &properties);
            ...
            if (vkCreateSampler(DeviceManager::logicalDevice.GetDevice(), &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS)
            {
                throw std::runtime_error("failed to create texture sampler!");
            }
        }

        VkImage Window::CreateTextureImage(std::string path, uint32_t mipLevels, VkDeviceMemory textureImageMemory, VkCommandPool& commandPool)
        {
            ...
            void* data{};
            vkMapMemory(DeviceManager::logicalDevice.GetDevice(), stagingBufferMemory, 0, imageSize, 0, &data);
        ...
        void Window::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels, VkCommandPool& commandPool)
        {
            // Refer to - https://vulkan-tutorial.com/en/Generating_Mipmaps

            // Check if image format supports linear blitting
            VkFormatProperties formatProperties;
            vkGetPhysicalDeviceFormatProperties(DeviceManager::physicalDevice.GetDevice(), imageFormat, &formatProperties);
        ...
        void Window::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels, VkCommandPool& commandPool)
        {
            // Refer to - https://vulkan-tutorial.com/en/Texture_mapping/Images
        ...
        void Window::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
        {
            ...
            if (vkCreateBuffer(DeviceManager::logicalDevice.GetDevice(), &bufferInfo, nullptr, &buffer) != VK_SUCCESS)
            {
                throw std::runtime_error("failed to create vertex buffer!");
            }

            VkMemoryRequirements memRequirements{};
            vkGetBufferMemoryRequirements(DeviceManager::logicalDevice.GetDevice(), buffer, &memRequirements);
            ...
            if (vkAllocateMemory(DeviceManager::logicalDevice.GetDevice(), &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS)
            {
                throw std::runtime_error("failed to allocate vertex buffer memory!");
            }

            vkBindBufferMemory(DeviceManager::logicalDevice.GetDevice(), buffer, bufferMemory, 0);
        }

        void Window::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, VkCommandPool& commandPool)
        {
            VkCommandBuffer commandBuffer = Helper::BeginSingleTimeCommands(commandPool);
        ...
        void Window::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, VkCommandPool& commandPool)
        {
            VkCommandBuffer commandBuffer = Helper::BeginSingleTimeCommands(commandPool);
        ...
        void Window::InsertImageMemoryBarrier(VkCommandBuffer commandBuffer, VkImage image, VkAccessFlags srcAccessMask, VkAccessFlags dstAccessMask, VkImageLayout oldImageLayout, VkImageLayout newImageLayout, VkPipelineStageFlags srcStageMask, VkPipelineStageFlags dstStageMask, VkImageSubresourceRange subresourceRange)
        {
