...
namespace FlatEngine
{
    ...
    std::vector<const char*> ValidationLayers::GetRequiredExtensions()
    {
        ...
        // return extensions;
        uint32_t ui_sdlExtensionCount = 0;
        std::vector<const char*> extensions;

        // Get required extensions from SDL
        SDL_Vulkan_GetInstanceExtensions(RenderWindow::window.GetWindow(), &ui_sdlExtensionCount, nullptr);
        extensions.resize(ui_sdlExtensionCount);
        SDL_Vulkan_GetInstanceExtensions(RenderWindow::window.GetWindow(), &ui_sdlExtensionCount, extensions.data());
