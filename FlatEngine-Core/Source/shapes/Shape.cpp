...
namespace FlatEngine
{
	...
	json Shape::GetShapeData()
	{
		...
		json jsonData = {
			...
			{ "b_enableContactEvents", m_shapeProps.b_enableContactEvents },
			{ "b_enableSensorEvents", m_shapeProps.b_enableSensorEvents },
			{ "b_isSensor", m_shapeProps.b_isSensor },
			...
			{ "b_horizontal", m_shapeProps.b_horizontal },
			{ "points", pointArray },
			{ "b_isLoop", m_shapeProps.b_isLoop },
	...
	{
		m_shapeProps.rotationOffset = b2MakeRot(Numbers::DegreesToRadians(Transform::ClampRotation(rotationOffset)));
		RecreateShape();
	}
