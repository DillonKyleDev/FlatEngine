...
function handleMovement()
    ...
    if data.mappingContext:ActionPressed("IA_MoveForward") then
        ...
        data.rigidBody:AddForce(Vector2:new(0,1), 10)
    ...
    if data.mappingContext:ActionPressed("IA_MoveRight") then        
        b_movingRight = true
        data.rigidBody:AddForce(Vector2:new(1,0), 10)
    ...
    if data.mappingContext:ActionPressed("IA_Shoot") then       
        local ball = Instantiate("SmallBall", data.transform:GetPosition())
        ball:GetRigidBody():AddForce(Vector2:new(1,1), 10)        
    end
